...
router.get('/', async (req, res) => {
    ...
    const customer = await mainPageModel.count('KhachHang', 'MaKH');
    
    // Tính tổng tiền của hóa đơn
    const detail = await mainPageModel.load('ChiTietHoaDon', 'SoCTHD');
    let revenue = 0;
    for (let index = 0; index < detail.length; index++) {
        let book = await mainPageModel.getPrice(detail[index].MaSach);
        revenue += parseInt(book[0].DonGia) * detail[index].SoLuong;       
    }

    // Tính doanh thu thực tế thu được từ phiếu thu tiền
    const getMoney = await mainPageModel.load('PhieuThuTien', 'MaPTT');
    let realRevenue = 0;
    ...
    const debt = await mainPageModel.count('PhieuGhiNo', 'MaPGN');

    // Tính số sách
    const book = await mainPageModel.count('Sach', 'MaSach');
    const bookKind = await mainPageModel.count('Sach', 'TheLoai');
